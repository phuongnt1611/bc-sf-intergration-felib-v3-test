...
public class BasePage {
	...
	public void selectItemInCustomDropdown(WebDriver driver, String xpathLocatorParent, String xpathLocatorAllItems,
			String textExpected) {
		getWebElement(driver, xpathLocatorParent).click();

		WebDriverWait explicitWait = new WebDriverWait(driver, longTimeout);

		explicitWait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(getByXpath(xpathLocatorAllItems)));
		...
		for (WebElement itemElement : listAllElement) {
			if (itemElement.getText().trim().equals(textExpected)) {
